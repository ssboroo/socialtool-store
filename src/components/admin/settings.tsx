    ...
    <div className="space-y-5 max-w-3xl">
      ...
      <section className="space-y-3 rounded-2xl border border-[#D6E4FF] bg-white p-5">
        ...
        <div className="space-y-3 rounded-xl border p-4">
          <label className="block text-sm font-semibold">Баннерын төрөл<select className="mt-2 block w-full rounded border bg-background p-2" value={values.heroMediaType||'images'} onChange={e=>setValues(v=>({...v,heroMediaType:e.target.value}))}><option value="images">Зургийн слайд</option><option value="video">Видео</option></select></label>
          <label className="block text-sm font-semibold">Видео холбоос (HTTPS MP4 / WebM)<Input type="url" value={values.heroVideoUrl||''} placeholder="https://example.com/banner.mp4" onChange={e=>setValues(v=>({...v,heroVideoUrl:e.target.value}))}/></label>
          <p className="text-sm text-muted-foreground">Видео нь шууд нээгддэг MP4 эсвэл WebM холбоос байна. YouTube хуудасны холбоос биш. 16:9 өргөн зураг/видео тохиромжтой. Видео дуугүй тоглогдоно; эхний зураг нь постер болно. Өөрчлөлтөө хадгална уу.</p>
        </div>
