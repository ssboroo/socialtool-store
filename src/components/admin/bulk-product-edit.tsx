...
export function BulkProductEdit({products,categories,token,onClose,onSaved}:{products:QuickProduct[];categories:{id:string;name:string}[];token:string;onClose:()=>void;onSaved:()=>void}){
  ...
  const patch:BulkPatch={...(categoryId?{categoryId}:{}),...(available?{available:available==='true'}:{}),...(featured?{featured:featured==='true'}:{}),...(templateEnabled?{shortDescTemplate:template}:{}),...(priceMode?{price:{mode:priceMode as 'set'|'percent',value:Number(value),includeVariants:variants}}:{})}
  let error='',rows:{p:QuickProduct;next:Record<string,unknown>}[]=[]
  try{if(priceMode&&!value.trim())throw new Error('Үнийн дүн оруулна уу');rows=products.map(p=>({p,next:bulkProductData(p,patch,categories.find(c=>c.id===categoryId))}))}catch(e){error=e instanceof Error?e.message:'Алдаа'}
  const save=async()=>{
  ...
  return <Dialog open onOpenChange={o=>{if(!o&&!saving)onClose()}}><DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto"><DialogTitle>{products.length} бүтээгдэхүүнийг бөөнөөр засах</DialogTitle>
    <fieldset disabled={saving} onChange={()=>setApproved(false)} className="grid gap-4 sm:grid-cols-2">
      ...
      <label>Үнэ<select className="block w-full rounded border bg-background p-2" value={priceMode} onChange={e=>setPriceMode(e.target.value)}><option value="">Өөрчлөхгүй</option><option value="set">Ижил үнэ тогтоох (₮)</option><option value="percent">Одоогийн үнээс хувиар өөрчлөх</option></select></label>
      {priceMode&&<><label>{priceMode==='set'?'Шинэ үнэ (₮)':'Өөрчлөх хувь (+ / −)'}<Input type="number" value={value} onChange={e=>setValue(e.target.value)}/></label><label className="self-center"><input type="checkbox" checked={variants} onChange={e=>setVariants(e.target.checked)}/> Эрхийн хувилбарын үнэд мөн үйлчлэх</label><p className="text-xs text-muted-foreground sm:col-span-2">Хуучин үнэ шинэ үнээс бага буюу тэнцүү бол хуучин үнэ, хямдралын тэмдэглэгээг цэвэрлэнэ. Хувийн өөрчлөлт нь худалдан авах өртгөөс бус, одоогийн үнээс тооцогдоно.</p></>}
