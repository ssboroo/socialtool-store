    ...
    <fieldset className={compact ? 'mt-2 min-w-0' : 'mt-4 min-w-0'} onClick={(e) => e.stopPropagation()}>
      ...
      >
        {options.map((term) => (
          <label
            key={term}
            className={`${compact ? 'rounded-[8px] px-1.5 py-1.5 text-[9px] leading-3' : 'rounded-lg px-2 py-2 text-xs leading-4'} min-w-0 cursor-pointer text-center font-bold transition-colors has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-blue-500 ${value === term ? 'bg-white text-[#0B4DBA] shadow-sm ring-1 ring-[#C7DCFA]' : 'text-[#698099] hover:bg-white/70'}`}
          >
