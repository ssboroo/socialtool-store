...
export function LicenseSelector({ value, onChange, options }: { options: string[]; value: LicenseTerm; onChange: (value: LicenseTerm) => void }) {
  ...
  return <fieldset className="mt-4 min-w-0" onClick={e => e.stopPropagation()}>
    ...
    <div className="grid grid-cols-2 gap-1 rounded-xl bg-[#EEF4FF] p-1">
      {options.map(term => <label key={term} className={`cursor-pointer rounded-lg px-2 py-2 text-center text-xs font-semibold transition-colors has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-blue-500 ${value === term ? 'bg-white text-[#0B4DBA] shadow-sm' : 'text-[#5B7290] hover:bg-white/60'}`}>
