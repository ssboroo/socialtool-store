...
import { createPaymentIntent, createCheckoutSession, retrievePaymentIntent, WireApiError, toMinorUnits } from '@/lib/wire'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!Number.isSafeInteger(order.totalAmount) || order.totalAmount <= 0) return NextResponse.json({ error: 'Төлбөрийн дүн буруу байна' }, { status: 400 })

    const previousIntentId = order.payment?.wirePaymentIntentId
    let expired = false
    if (previousIntentId) {
      const current = await retrievePaymentIntent(previousIntentId)
      const expectedAmount = toMinorUnits(order.totalAmount)
      const amountMismatch = current.currency !== 'MNT' || current.amount !== expectedAmount
      const alreadyPaid = current.status === 'succeeded' || current.status === 'paid'

      if (alreadyPaid && !amountMismatch) {
      ...
      if (alreadyPaid && amountMismatch) {
        return NextResponse.json({ error: 'Төлөгдсөн гүйлгээний дүн захиалгын дүнтэй зөрж байна. Админтай холбогдоно уу.', code: 'amount_mismatch' }, { status: 409 })
      }

      // Recreate old pending intents generated by the previous ×100 MNT conversion.
      expired = amountMismatch || ['canceled', 'cancelled', 'expired', 'failed'].includes(current.status)
