...
async function readCart(customerId: string) {
  ...
  const saved = JSON.parse(customer.cartItems) as { id: string; quantity: number }[]
  ...
  return {
    ...
    items: saved.flatMap(item => {
      ...
      return p ? [{ id: p.id, name: p.name, price: p.price, icon: p.icon, category: p.category, quantity: item.quantity }] : []
    }),
  }
}
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    if (!Number.isSafeInteger(version) || version < 0 || !Array.isArray(items) || items.length > 100 || items.some(i => !i || typeof i.id !== 'string' || !Number.isSafeInteger(i.quantity) || i.quantity < 1 || i.quantity > 99)) {
      return NextResponse.json({ error: 'Сагсны мэдээлэл буруу байна' }, { status: 400 })
    }
    if (new Set(items.map(i => i.id)).size !== items.length) return NextResponse.json({ error: 'Давхардсан бүтээгдэхүүн байна' }, { status: 400 })
    const result = await db.customer.updateMany({
      where: { id: customer.sub, cartVersion: version },
      data: { cartItems: JSON.stringify(items.map(i => ({ id: i.id, quantity: i.quantity }))), cartVersion: { increment: 1 } },
    })
