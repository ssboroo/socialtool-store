...
test('local production API: registration, admin, image upload and account-isolated carts', async () => {
  ...
  try {
    ...
    const freeListing=await (await request('/api/products?free=1')).json()
    assert.ok(freeListing.some(p=>p.id===free.id))
    assert.ok(freeListing.every(p=>p.price===0&&p.downloadUrl))
    assert.equal((await (await request('/api/products/'+free.id)).json()).downloadUrl,freeBody.downloadUrl)
    const freeUpdate=data=>request('/api/admin/products/'+free.id,{...json(data),method:'PUT',headers:{...adminHeaders,'Content-Type':'application/json'}})
    assert.equal((await freeUpdate({price:100})).status,400)
    assert.equal((await freeUpdate({downloadUrl:'http://example.org/unsafe'})).status,400)
    assert.equal((await freeUpdate({shortDesc:'Free download'})).status,200)
    const freeCurrent=await db.product.findUnique({where:{id:free.id}})
    assert.equal((await request('/api/admin/products/bulk-edit',{...json({items:[{id:free.id,updatedAt:freeCurrent.updatedAt.toISOString()}],patch:{price:{mode:'set',value:100,includeVariants:false}}}),method:'PUT',headers:{...adminHeaders,'Content-Type':'application/json'}})).status,400)
    assert.equal((await request('/api/orders',json({customerName:'Test',phone:'00000000',email:accountA.email,items:[{productId:free.id,name:free.name,price:100,quantity:1}]},cookieA))).status,400)
    console.log('Verified free program CRUD, HTTPS validation, paid/free invariant, filter and checkout/bulk-price protection')
    ...
    const paidHistory=await request('/api/customer/orders',{headers:{cookie:cookieA}})
    assert.match(paidHistory.headers.get('cache-control'),/no-store/)
    assert.equal((await paidHistory.json()).find(o=>o.id===order.id).status,'PAID')
    assert.deepEqual(await (await request('/api/customer/orders',{headers:{cookie:cookieB}})).json(),[])
    await db.order.update({ where:{id:order.id}, data:{status:'DELIVERED'} })
    assert.equal((await (await request('/api/customer/orders',{headers:{cookie:cookieA}})).json()).find(o=>o.id===order.id).status,'DELIVERED')
    ...
    const peer = await db.product.create({data:{name:'Bulk peer',slug:suffix+'-bulk',shortDesc:'',description:'',price:100,category:category.name,categoryId:category.id}})
    const beforeBulk = await db.product.findUnique({where:{id:product.id}})
    const bulk = (items, authorized=true) => request('/api/admin/products/bulk-images',{...json({items}),method:'PUT',headers:{'Content-Type':'application/json',...(authorized?adminHeaders:{})}})
    const rows = [{id:product.id,previousImage:beforeBulk.image,image:'/products/default/ai.svg'},{id:peer.id,previousImage:null,image:'/products/default/windows.svg'}]
    assert.equal((await bulk(rows,false)).status,401)
    assert.equal((await bulk(rows)).status,200)
    assert.equal((await db.product.findUnique({where:{id:product.id}})).image,rows[0].image)
    assert.equal((await db.product.findUnique({where:{id:peer.id}})).image,rows[1].image)
    assert.equal((await bulk([{id:product.id,previousImage:rows[0].image,image:'/products/default/facebook.svg'},{id:peer.id,previousImage:null,image:'/products/default/facebook.svg'}])).status,409)
    assert.equal((await db.product.findUnique({where:{id:product.id}})).image,rows[0].image)
    assert.equal((await bulk([{id:product.id,previousImage:rows[0].image,image:'javascript:alert(1)'}])).status,400)
    assert.equal((await bulk([{id:product.id,previousImage:rows[0].image,image:uploadedUrl}])).status,200)
    const sitemap = await request('/sitemap.xml')
    assert.equal(sitemap.status,200)
    const sitemapText = await sitemap.text()
    assert.ok(sitemapText.includes('<urlset'))
    assert.ok(!sitemapText.includes('/admin'))
    assert.equal((await request('/robots.txt')).status,200)
    assert.equal((await request('/icon.svg')).status,200)
    console.log('Verified bulk images: auth, two-product save, atomic conflict rollback, invalid-image rejection; sitemap, robots and favicon routes')
